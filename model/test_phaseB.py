...
""" Reminder: The process up to this point needs to be done twice, once for a real and once for a synthetic dataset.
The .csv file that contains the extracted features from the real model is used and the features extracted from the 
...
for i in range (len(reconstructed_ds_r2)):
    ...
    features_r[24] = core_psnr_r2[i]
    features_r[25] = core_ssim_r2[i]
    features_r[26] = core_msssim_r2[i]
    ...
    features_r[51] = core_psnr_r22[i]
    features_r[52] = core_ssim_r22[i]
    features_r[53] = core_msssim_r22[i]
    features_r[54] = label_f
    features.append(features_r)

features_fake = np.asarray(features)

""" Reads the training data from test phase A. """
featurestr = np.loadtxt("Path to the .csv file saved from test phase A", delimiter=',')
X_train = featurestr[:,0:54]
y_train = featurestr[:,54]

""" Reads the testing data from phase B. """
X_test = features_fake[:,0:54]
y_test = features_fake[:,54]
   

""" The binary classifier. """

# Defines the detection model.
model_det = Sequential()
model_det.add(Dense(12, input_shape=(54,), activation='relu'))
model_det.add(Dense(8, activation='relu'))
model_det.add(Dense(1, activation='sigmoid'))

# Compiles the detection model.
model_det.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy', tf.keras.metrics.Precision(), tf.keras.metrics.Recall(), tf.keras.metrics.AUC()])

# Fits the detection model on the dataset.
model_det.fit(X_train, y_train, epochs=300, batch_size=16)                # Default batch size is 16.

# Evaluates the detection model.
_, accuracy, precision, recall, auc = model_det.evaluate(X_test, y_test)
